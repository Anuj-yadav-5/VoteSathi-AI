...
export function renderNavbar() {
  return `
    <header style="position:sticky;top:0;z-index:100;background:rgba(12,10,8,0.92);backdrop-filter:blur(20px);border-bottom:1px solid rgba(245,158,11,0.12);">
      ...
      <div class="container" style="display:flex;align-items:center;justify-content:space-between;height:62px;">
        ...
        <a href="#home" aria-label="Go to Home" onclick="window.resetHomeChat?.();" style="display:flex;align-items:center;gap:10px;text-decoration:none;">
          <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg, #f59e0b, #d97706);display:flex;align-items:center;justify-content:center;color:white;box-shadow:0 4px 12px rgba(245,158,11,0.3);">
            <i data-lucide="vote" style="width:20px;height:20px;"></i>
          </div>
          <span style="font-family:var(--font-outfit);font-weight:800;font-size:17px;color:var(--color-text);letter-spacing:-0.3px;">VoteSathi <span style="background:linear-gradient(135deg,#fbbf24,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">AI</span></span>
        </a>
        ...
        <!-- Right: Helpline pill + Mobile menu -->
      ...
      <!-- Mobile Drawer -->
      <div id="mobile-drawer" style="display:none;flex-direction:column;padding:16px 24px 24px;gap:4px;border-top:1px solid rgba(245,158,11,0.12);background:rgba(12,10,8,0.98);">
        ${NAV_ITEMS.map((i) => `
